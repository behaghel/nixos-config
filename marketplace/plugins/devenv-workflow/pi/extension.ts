...
This project may also configure other coding agents via \`devenv.nix\`:
- **Claude Code** — config at \`.claude/\` (managed by devenv)
- **OpenCode** — config at \`.opencode/\` (managed by devenv)
- **Pi** — config at \`.pi/\` (managed by \`pi install\`)

These are **independent layers**. Do not modify one agent’s configuration from another’s tooling:
- Claude/OpenCode agent config is generated by \`devenv\` — edit \`devenv.nix\`, not the generated files.
- Pi extensions are installed via \`pi install -l <path>\`, not via \`devenv.nix\`.
`;
