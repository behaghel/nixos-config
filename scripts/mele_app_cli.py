...
def find_repo_root(start: Path | None = None) -> Path:
    ...
    for candidate in candidates:
        if (candidate / "flake.nix").is_file() and (candidate / MELE_HUB_DIR).is_dir():
            return candidate
    raise CliError(
        "could not find nixos-config repo root; run from a checkout containing "
        f"flake.nix and {MELE_HUB_DIR}"
    )


def validate_static_site_name(name: str) -> None:
    if not STATIC_SITE_NAME_RE.match(name):
        raise CliError(
            "invalid static site name; use lowercase letters, numbers, and "
...
def validate_mele_eval(repo_root: Path) -> None:
    print("validating MeLE config...")
    result = capture_command([
        ...
        "false",
        "--raw",
        ".#nixosConfigurations.mele-hub.config.system.build.toplevel.drvPath",
    ], cwd=repo_root)
    if result.returncode != 0:
        output = (result.stdout + result.stderr).strip()
        raise CliError(f"MeLE NixOS evaluation failed: {output}")
...
def cmd_static_create(args: argparse.Namespace) -> int:
    ...
    site_dir = repo_root / STATIC_SITE_DIR
    if not (repo_root / "configurations/nixos/mele-hub/static-sites.nix").is_file():
    ...
    print(f"root: /srv/static/{args.name}/current")
    print(f"placeholder: https://{domain} shows 'Soon here…' after activation until first deploy")
    print("\nRecommended project initialization:")
    print(
        f"  om init --non-interactive --params '{{\"site-name\":\"{args.name}\"}}' "
        f"-o ~/ws/{args.name} {repo_root}#hugo-ox-static-site"
    )
    print("\nThis single value derives baseURL, MeLE site, and deploy root:")
    print(f"  site-name: {args.name}")
    print("\nFallback without Omnix:")
    print(f"  nix flake new ~/ws/{args.name} --template {repo_root}#hugo-ox-static-site")
    print(f"  # then replace example -> {args.name}")
    print(f"guide: {STATIC_SITE_GUIDE}")
    return 0


def dispatch_static(args: argparse.Namespace) -> int:
    if args.static_command == "create":
        return cmd_static_create(args)
    raise CliError(f"unimplemented static command: {args.static_command}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    static = subparsers.add_parser("static", help="Manage static site slots")
    static_subparsers = static.add_subparsers(
        dest="static_command",
        required=True,
    )
    static_create = static_subparsers.add_parser("create")
    static_create.add_argument("name")
    static_create.add_argument("--domain")
    static_create.add_argument("--force", action="store_true")
    static_create.add_argument(
        "--no-validate",
        action="store_true",
